...
<template>
  <header
    class="w-8/12 max-[530px]:w-16 max-[530px]:top-2 max-[530px]:left-10 py-2 px-4 rounded-full transition-all duration-500 flex items-center justify-between m-auto font-KellySlab fixed text-white z-50 top-[5%] left-1/2 -translate-x-1/2"
    id="header"
    v-if="!menu"
  >
    <div
    ...
    >
      <img src="../glob/hamburger.svg" alt="" class="w-12 h-12" />
    </div>
  </header>
  <div
    class="fixed w-screen h-screen bg-[#00000033] backdrop-blur-xl z-40 animate-[openMenu_1s_ease] top-0 left-0 float-left overflow-x-hidden"
    v-if="menu"
    id="menu"
  >
    <div
    ...
    >
      <div class="py-2 px-4 rounded-full transition-all duration-500 relative">
        <img src="../glob/close.svg" alt="" class="w-10 h-10 top-4 left-4 absolute" />
        
      </div>
      <div
        class="flex items-center justify-center w-full h-full flex-col text-white font-Ubuntu gap-6 text-xl animate-[openMenu_1s_ease]"
      >
